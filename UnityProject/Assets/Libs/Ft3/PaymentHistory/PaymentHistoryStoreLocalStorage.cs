...
namespace Chromia.Postchain.Ft3
{
  public class PaymentHistoryStoreLocalStorage : PaymentHistoryStore
  {
    ...
    public void Save(byte[] accountId, PaymentHistoryEntry[] paymentHistoryEntries)
    {
      ...
      this._entriesCache[PostchainUtil.ByteArrayToString(accountId).ToUpper()] = newEntries;
    }
    ...
    {
      string key = "FT3_LIB_P_H_S_I_" + PostchainUtil.ByteArrayToString(accountId).ToUpper();
      dynamic value;
      try
      {
    ...
    {
      string accountIdString = PostchainUtil.ByteArrayToString(accountId).ToUpper();
    ...
    {
      string key = "FT3_LIB_P_H_" + PostchainUtil.ByteArrayToString(id).ToUpper();
      dynamic value;
      try
      {
        value = _localStorage.Get(key);
    ...
    {
      string key = "FT3_LIB_P_H" + PostchainUtil.ByteArrayToString(id).ToUpper();
    ...
    private PaymentHistoryEntry MapToPaymentHistoryEntry(dynamic entry)
    {
      return new PaymentHistoryEntry(
          ...
          (int)entry["delta"],
          (string)entry["asset"],
          PostchainUtil.HexStringToBuffer((string)entry["assetId"]),
          (dynamic[])entry["other"],
          (string)entry["timestamp"],
          PostchainUtil.HexStringToBuffer((string)entry["transactionId"]),
