...
namespace Chromia.Postchain.Ft3
{
  public class Asset
  {
    ...
    public static async Task<Asset> Register(string name, byte[] chainId, Blockchain blockchain)
    {
      ...
      tx.AddOperation("ft3.dev_register_asset", name, PostchainUtil.ByteArrayToString(chainId));
    ...
    public static async Task<Asset[]> GetByName(string name, Blockchain blockchain)
    {
      ...
      foreach (var asset in assets.content)
      {
        assetList.Add(
            new Asset(
                (string)asset["name"],
                PostchainUtil.HexStringToBuffer((string)asset["issuing_chain_rid"])
            )
        );
      }
    ...
    {
      var asset = await blockchain.Connection.Gtx.Query<dynamic>("ft3.get_asset_by_id", ("asset_id", PostchainUtil.ByteArrayToString(id)));
      return new Asset((string)asset.content["name"], PostchainUtil.HexStringToBuffer((string)asset.content["issuing_chain_rid"]));
    }
