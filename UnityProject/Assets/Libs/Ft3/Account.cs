...
namespace Chromia.Postchain.Ft3
{
  ...
  public class Account
  {
    ...
    public static async Task<Account[]> GetByParticipantId(byte[] id, BlockchainSession session)
    {
      ...
      (dynamic content, PostchainErrorControl control) accountIds = await session.Query<dynamic>(gtv[0] as string, (System.ValueTuple<string, string>)gtv[1]);


      var idList = new List<byte[]>();
      foreach (var accountId in accountIds.content)
      {
        idList.Add(PostchainUtil.HexStringToBuffer((string)accountId));
      }
    ...
    public static async Task<Account> GetById(byte[] id, BlockchainSession session)
    {
      ...
      (dynamic content, PostchainErrorControl control) account = await session.Query<dynamic>(gtv[0] as string, (System.ValueTuple<string, string>)gtv[1]);
    ...
    private async Task SyncAuthDescriptors()
    {
      ...
      (dynamic content, PostchainErrorControl control) authDescriptors = await this.Session.Query<dynamic>(authGtv[0] as string, (System.ValueTuple<string, string>)authGtv[1]);
      ...
      foreach (var authDescriptor in authDescriptors.content)
      {
        authList.Add(
            authDescriptorFactory.Create(
                ...
                PostchainUtil.HexStringToBuffer((string)authDescriptor["args"])
            )
        );
      }
    ...
    {
      return this.Assets.Find(assetBalance => PostchainUtil.ByteArrayToString(assetBalance.Asset.GetId()).Equals(PostchainUtil.ByteArrayToString(id)));
    }
