...
namespace Chromia.Postchain.Ft3
{
  public class Blockchain
  {
    ...
    public async static Task<Blockchain> Initialize(byte[] blockchainRID, DirectoryService directoryService)
    {
      ...
      {
        throw new Exception("Cannot find details for chain with RID: " + PostchainUtil.ByteArrayToString(blockchainRID));
      }

      var connection = new ConnectionClient(
          chainConnectionInfo.Url,
          PostchainUtil.ByteArrayToString(blockchainRID)
      );
    ...
    public async Task LinkChain(byte[] chainId)
    {
      ...
      tx.AddOperation("ft3.xc.link_chain", PostchainUtil.ByteArrayToString(chainId));
    ...
    public async Task<bool> IsLinkedWithChain(byte[] chainId)
    {
      var info = await this.Query<int>(
          ...
          ("chain_rid", PostchainUtil.ByteArrayToString(chainId))
      );
    ...
    public async Task<List<byte[]>> GetLinkedChainsIds()
    {
      ...
      return linkedChains.content.Select(elem => PostchainUtil.HexStringToBuffer(elem)).ToList();
    }
